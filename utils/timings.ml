...
let timings : (part, data * data option) Hashtbl.t = Hashtbl.create 20
...
let start part =
  ...
  let { Gc.minor_words } = Gc.quick_stat () in
  Hashtbl.add timings part ({ time; minor_words }, None)
...
let stop part =
  ...
  let stop =
    { time = time -. start.time;
      minor_words = minor_words -. start.minor_words } in
  Hashtbl.replace timings part (start, Some stop)
...
let restart part =
  let previous_duration =
    ...
    | exception Not_found -> { time = 0.; minor_words = 0. }
    | (_, Some duration) -> duration
    | _, None -> assert false
  in
  let { Gc.minor_words } = Gc.quick_stat () in
  let time = Sys.time () in
  Hashtbl.replace timings part ({ time; minor_words }, Some previous_duration)
...
let accumulate part =
  ...
  | _, None -> assert false
  | (start, Some accumulated) ->
    let stop =
      { time = accumulated.time +. (time -. start.time);
        minor_words = accumulated.minor_words +.
                      (minor_words -. start.minor_words) }
    in
    Hashtbl.replace timings part (start, Some stop)
...
let get_time part =
  match Hashtbl.find timings part with
  | _start, Some duration -> Some duration.time
  | _, None -> None
  | exception Not_found -> None

let get_allocations part =
  match Hashtbl.find timings part with
  | _start, Some duration -> Some duration.minor_words
  | _, None -> None
  | exception Not_found -> None
...
let part_name = function
  ...
  | Flambda_pass (pass, file) -> Printf.sprintf "flambda-pass-%s(%s)" pass file
...
let timings_list () =
  ...
  List.sort (fun (_, (start1, _)) (_, (start2, _)) -> compare start1.time start2.time) l
...
let print ppf =
  ...
  List.iter (fun (part, (start, stop)) ->
      match stop with
      | Some duration ->
        Format.fprintf ppf "%s: %.03fs@." (part_name part) duration.time;
        Format.fprintf ppf "%s_alloc: %.00f@." (part_name part) duration.minor_words;
      | None ->
