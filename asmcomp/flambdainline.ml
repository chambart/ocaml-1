...
let should_inline_function_known_to_be_recursive ~func ~clos ~env ~closure
  ...
  (* CR mshinwell: I'm not sure whether some of the predicate passed to
     ...
     pchambart: having an useful approximation with the right to use it
     should probably be essential for now: otherwise we known that the
     duplicated fonction will be exactly identical to the original one.
     But we could allow a bit more cases by being a bit more laxist on what
     can be specialised: recursive calls not keeping arguments could call
     the original function. But that would need a difficult choice about
     which parameter do we choose to specialise. For instance if we want
     to specialise this function, there is no obvious choice.
     [let rec f x y =
        if x
        then f (not x) y
        else f x (y-1)]
...
and direct_apply env r clos funct fun_id func fapprox closure
  ...
  *)
  ...
  | _, Flambdacost.Never_inline ->  (* We decided not to inline it. *)
      no_transformation ()
  | _, ((Flambdacost.Can_inline _) as remaining_inline_threshold) ->
      (* CR pchambart for mshinwell: The inline requirement Always should
         probably also be taken into account here, even if the preliminary
         cost is considered too high. The second check is the safeguard
         against size explosion *)
      ...
      (* CXR mshinwell for pchambart: two variables called [threshold] and
         [inline_threshold] is confusing.
         pchambart: replaced by remaining_inline_threshold
         *)
