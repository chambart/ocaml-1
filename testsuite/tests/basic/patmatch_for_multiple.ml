    ...
    (catch
      (catch
        (if (!= *match*/94 3) (exit 6)
          (let
            (x/97 : [0: int, int, int] =a
               (makeblock 0 *match*/93 *match*/94 *match*/95))
            (exit 4 x/97)))
       with (6)
        (if (!= *match*/93 1) (exit 5)
          (let
            (x/96 : [0: int, int, int] =a
               (makeblock 0 *match*/93 *match*/94 *match*/95))
            (exit 4 x/96))))
     with (5) 0)
   with (4 x/91[0: int, int, int]) (seq (ignore x/91) 1)))
(let (*match*/93 = 3 *match*/94 = 2 *match*/95 = 1)
  (catch
    (if (!= *match*/94 3)
      (if (!= *match*/93 1) 0
...
(* outside, trivial *)
[%%expect {|
(function a/102[int] b/103 : [0: int, *]
  (let (p/104 : [0: int, *] =a (makeblock 0 a/102 b/103)) p/104))
(function a/102[int] b/103 : [0: int, *] (makeblock 0 a/102 b/103))
- : bool -> 'a -> bool * 'a = <fun>
|}]
...
(* inside, trivial *)
[%%expect{|
(function a/106[int] b/107 : [0: int, *]
  (let (p/108 : [0: int, *] =a (makeblock 0 a/106 b/107)) p/108))
(function a/106[int] b/107 : [0: int, *] (makeblock 0 a/106 b/107))
- : bool -> 'a -> bool * 'a = <fun>
|}];;
...
(* outside, simple *)
[%%expect {|
(function a/112[int] b/113 : [0: int, [0: *, *]]
  (let
    (x/114 : int =a a/112 p/115 : [0: int, *] =a (makeblock 0 a/112 b/113))
    (makeblock 0 (int,[0: int, *]) x/114 p/115)))
(function a/112[int] b/113 : [0: int, [0: *, *]]
  (makeblock 0 (int,[0: int, *]) a/112 (makeblock 0 a/112 b/113)))
...
(* inside, simple *)
[%%expect {|
(function a/118[int] b/119 : [0: int, [0: *, *]]
  (let
    (x/120 : int =a a/118 p/121 : [0: int, *] =a (makeblock 0 a/118 b/119))
    (makeblock 0 (int,[0: int, *]) x/120 p/121)))
(function a/118[int] b/119 : [0: int, [0: *, *]]
  (makeblock 0 (int,[0: int, *]) a/118 (makeblock 0 a/118 b/119)))
- : bool -> 'a -> bool * (bool * 'a) = <fun>
|}]
...
(* outside, complex *)
[%%expect{|
(function a/128[int] b/129[int] : [0: int, [0: *, *]]
  (if a/128
    (let
      (x/130 : int =a a/128
       p/131 : [0: int, int] =a (makeblock 0 a/128 b/129))
      (makeblock 0 (int,[0: int, int]) x/130 p/131))
    (let
      (x/132 : [0: ] =a b/129
       p/133 : [0: int, int] =a (makeblock 0 a/128 b/129))
      (makeblock 0 (int,[0: int, int]) x/132 p/133))))
(function a/128[int] b/129[int] : [0: int, [0: *, *]]
  (if a/128 (makeblock 0 (int,[0: int, int]) a/128 (makeblock 0 a/128 b/129))
    (makeblock 0 (int,[0: int, int]) b/129 (makeblock 0 a/128 b/129))))
...
(* inside, complex *)
[%%expect{|
(function a/134[int] b/135[int] : [0: int, [0: *, *]]
  (catch
    (if a/134
      (let
        (x/142 : int =a a/134
         p/143 : [0: int, int] =a (makeblock 0 a/134 b/135))
        (exit 10 x/142 p/143))
      (let
        (x/140 : [0: ] =a b/135
         p/141 : [0: int, int] =a (makeblock 0 a/134 b/135))
        (exit 10 x/140 p/141)))
   with (10 x/136[int] p/137[0: int, int])
    (makeblock 0 (int,[0: int, int]) x/136 p/137)))
(function a/134[int] b/135[int] : [0: int, [0: *, *]]
  (catch
...
(* outside, onecase *)
[%%expect {|
(function a/144[int] b/145[int] : [0: int, [0: *, *]]
  (if a/144
    (let
      (x/146 : int =a a/144
       _p/147 : [0: int, int] =a (makeblock 0 a/144 b/145))
      (makeblock 0 (int,[0: int, int]) x/146 [0: 1 1]))
    (let
      (x/148 : int =a a/144
       p/149 : [0: int, int] =a (makeblock 0 a/144 b/145))
      (makeblock 0 (int,[0: int, int]) x/148 p/149))))
(function a/144[int] b/145[int] : [0: int, [0: *, *]]
  (if a/144 (makeblock 0 (int,[0: int, int]) a/144 [0: 1 1])
    (makeblock 0 (int,[0: int, int]) a/144 (makeblock 0 a/144 b/145))))
- : bool -> bool -> bool * (bool * bool) = <fun>
|}]
...
(* inside, onecase *)
[%%expect{|
(function a/150[int] b/151 : [0: int, [0: *, *]]
  (let
    (x/152 : int =a a/150 p/153 : [0: int, *] =a (makeblock 0 a/150 b/151))
    (makeblock 0 (int,[0: int, *]) x/152 p/153)))
(function a/150[int] b/151 : [0: int, [0: *, *]]
  (makeblock 0 (int,[0: int, *]) a/150 (makeblock 0 a/150 b/151)))
- : bool -> 'a -> bool * (bool * 'a) = <fun>
|}]
...
(* outside, tuplist *)
[%%expect {|
(function a/163[int] b/164 : [0: int, *]
  (catch
    (if a/163 (if b/164 (let (p/165 =a (field 0 b/164)) p/165) (exit 12))
      (exit 12))
   with (12) (let (p/166 : [0: int, *] =a (makeblock 0 a/163 b/164)) p/166)))
(function a/163[int] b/164 : [0: int, *]
...
(* inside, tuplist *)
[%%expect{|
(function a/167[int] b/168 : [0: int, *]
  (catch
    (catch
      (if a/167
        ...
        (exit 14))
     with (14)
      (let (p/171 : [0: int, *] =a (makeblock 0 a/167 b/168))
        (exit 13 p/171)))
   with (13 p/169[0: int, *]) p/169))
(function a/167[int] b/168 : [0: int, *]
  (catch
    (catch
