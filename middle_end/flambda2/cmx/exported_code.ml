...
type raw = { code : Code_or_metadata.raw Code_id.Map.t } [@@unboxed]

type t = { code : Code_or_metadata.t Code_id.Map.t } [@@unboxed]
...
let mark_as_imported t =
  let code =
    Code_id.Map.map_sharing Code_or_metadata.remember_only_metadata t.code
  in
  { code }

let merge { code = code1 } { code = code2 } =
...
let remove_unused_closure_vars_from_result_types ~used_closure_vars t : t =
  let code =
    Code_id.Map.map
      (fun code_or_metadata ->
        Code_or_metadata.map_result_types code_or_metadata ~f:(fun result_ty ->
            Flambda2_types.remove_unused_closure_vars result_ty
              ~used_closure_vars))
      t.code
  in
  { code }
...
let associate_with_loaded_cmx_file (t : raw) ~read_flambda_section_from_cmx_file
    ~code_sections_map ~used_closure_vars ~original_compilation_unit =
  let read_flambda_section_from_cmx_file ~index : Code_or_metadata.code_in_cmx =
    read_flambda_section_from_cmx_file ~index |> Obj.obj
  in
  ...
  let code =
    Code_id.Map.mapi
      (fun code_id code_or_metadata ->
        Code_or_metadata.associate_with_loaded_cmx_file code_or_metadata code_id
          loader)
      t.code
  in
  { code }
