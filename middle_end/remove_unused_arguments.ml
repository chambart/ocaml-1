...
let remove_params unused (fun_decl: Flambda.function_declaration) =
  let unused_params, used_params =
    List.partition (fun (v, _typ) -> Variable.Set.mem v unused) fun_decl.params
  in
  let unused_params = List.filter (fun (v, _typ) ->
      Variable.Set.mem v fun_decl.free_variables) unused_params
  in
  let body =
    List.fold_left (fun body (var, _typ) ->
...
let make_stub unused var (fun_decl : Flambda.function_declaration)
  ...
  let args' =
    List.map (fun (var, typ) -> var, rename_var var, typ) fun_decl.params
  in
  let used_args' =
    List.filter (fun (var, _, _) -> not (Variable.Set.mem var unused)) args'
  in
  let args_renaming = Variable.Map.of_list (List.map (fun (v1, v2, _) -> v1,v2) args') in
  let additional_specialised_args =
    List.fold_left (fun additional_specialised_args (original_arg,arg,_typ) ->
  ...
  in
  let function_decl =
    Flambda.create_function_declaration ~params:(List.map (fun (_, arg, typ) -> arg, typ) args') ~body
  ...
  let non_stub_arguments =
    Variable.Map.fold (fun _ (decl : Flambda.function_declaration) acc ->
        if decl.stub then
          ...
          Variable.Set.union acc (Variable.Set.of_list (List.map fst decl.Flambda.params)))
  ...
  then None
    ...
    let funs, additional_specialised_args =
      Variable.Map.fold (fun fun_id (fun_decl : Flambda.function_declaration)
          ...
          if List.exists (fun v -> Variable.Set.mem v unused) (List.map fst fun_decl.params)
          then begin
