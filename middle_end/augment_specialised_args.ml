  ...
  let new_specialised_arg t ~fun_var ~group ~definition =
    let for_one_function : for_one_function =
      ...
      | exception Not_found ->
        ...
        | (function_decl : Flambda.function_declaration) ->
          let params = Variable.Set.of_list (List.map fst function_decl.params) in
  ...
  let rename_function_and_parameters ~fun_var
    ...
    let params_renaming =
      Variable.Map.of_list
        (List.map (fun (param, _typ) ->
            let new_param = Variable.rename param ~append:T.variable_suffix in
            param, new_param)
          function_decl.params)
    in
    let renamed_params =
      List.map (fun (param, typ) -> Variable.Map.find param params_renaming, typ)
        function_decl.params
    in
      ...
      let all_params =
        let new_params =
          Variable.Set.elements (Variable.Map.keys
            for_one_function.new_inner_to_new_outer_vars)
        in
        function_decl.params @ List.map (fun v -> v, Flambda.Val) new_params
      in
