...
void caml_memprof_postpone_track_alloc_shr(value block) {
  ...
  if(occurences > 0) {
    struct postponed_block* pb =
      (struct postponed_block*)caml_stat_alloc(sizeof(struct postponed_block));
    if(pb == NULL) return;
    pb->block = block;
...
void caml_memprof_track_interned(header_t* block, header_t* blockend) {
  ...
  while(1) {
    ...
    if(sz == 0) {
      sz = 32;
      sampled = (value*)caml_stat_alloc(sizeof(value) * sz);
      if(sampled == NULL)
        CAMLreturn0;
      occurences = (int32_t*)caml_stat_alloc(sizeof(int32_t) * sz);
      if(occurences == NULL) {
      ...
      sz *= 2;
      newsampled = (value*)caml_stat_resize(sampled, sizeof(value) * sz);
      if(newsampled == NULL) {
      ...
      sampled = newsampled;
      newoccurences =
        (int32_t*)caml_stat_resize(occurences, sizeof(int32_t) * sz);
...
void caml_memprof_call_gc_end(double exceeded_by) {
  ...
  if(exceeded_by > 0) {
    ...
    samples = (struct smp*)caml_stat_alloc(sz*sizeof(struct smp));
    if(samples == NULL)
      goto abort;
    ...
    for(i = 0; i < num_blocks; i++) {
      ...
      if(next_sample < 0) {
        if(n_samples >= sz) {
          sz *= 2;
          p = (struct smp*)caml_stat_resize(samples, sz * sizeof(struct smp));
    ...
    ephes = (value*)caml_stat_alloc(n_samples * sizeof(value));
